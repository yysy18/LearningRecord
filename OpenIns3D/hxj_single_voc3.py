...
def single_vocabulary_detection(path_3D_scans, vocabulary, path_masks= None, path_images = None,detector="odise"):
    ...
    remove_lip=pcd_rgb[:, 2].max()-1.7
    print("111111111111111111111")
    print(pcd_rgb[:, 2].max())
    print(remove_lip)
    snap_class = Snap([800, 800], [2, 0.5, remove_lip], "output/hxj_snap_demo")
    ...
    if path_images:
        ...
        snap_class.scene_image_rendering(xyz_rgb, name_of_scene, mode=["hxj"])
    ...
    snap_class.scene_image_rendering(torch.tensor(xyz_rgb).float(), f"{name_of_scene}_vis", mode=["hxj"], mask=[mask_final, None])
