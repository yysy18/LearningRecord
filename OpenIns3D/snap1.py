...
class Snap:
    ...
    def get_rid_of_lip1(self, mesh, scan_pc, remove_lip):
        """
        Given a mesh file in PyTorch3D, remove the lip with a predefined height, and return a mesh file in PyTorch3D
        """
        verts = mesh.verts_packed()
    ...
    def scene_image_rendering(self, scan_pc_raw, scene_name, k,mode = ["global"], mask = None):
        ...
        generation_functions = {
            ...
            "right": self.right_angle_level_camera_generation
        }
 ...
 ######################################################hxj#######################################################
    def right_angle_level_camera_generation(self, w, l, h, scene_center, scan_pc):

        # Calculate fractions of dimensions
        third_width, third_length, third_height = w / 3, l / 3, h / 3
        half_width, half_length,half_height = w / 2, l / 2, h / 2

        # # Define top and bottom z-coordinates
        # top_z = scene_center[2] + half_height#top_z
        # bottom_z = scene_center[2] - half_height#bottom_z
        # # Calculate camera positions [4 by 4 line 1,1; 1,2; 2,1; 2,2]
        # camera_positions = [
        #     np.array([scene_center[0] - third_width/2, scene_center[1] - third_length/2, top_z]),
        #     np.array([scene_center[0] + third_width/2, scene_center[1] - third_length/2, top_z]),
        ...
        left= scene_center[0] + third_width #left
        right= scene_center[0] - third_width #right

        # top_z= scene_center[1] + third_length#forward
        # bottom_z= scene_center[1] - third_length#backward


        # Calculate camera positions [4 by 4 line 1,1; 1,2; 2,1; 2,2]
        camera_positions = [
            np.array([left,scene_center[1] - third_length/2, scene_center[2] - third_height/2]),
            np.array([left,scene_center[1] + third_length/2, scene_center[2] - third_height/2]),
            np.array([left,scene_center[1] - third_length/2, scene_center[2] + third_height/2]),
            np.array([left,scene_center[1] + third_length/2, scene_center[2] + third_height/2])
        ]
        ...
        # Calculate target positions
        target_positions = [
            np.array([right,scene_center[1] + half_length, scene_center[2] + half_height]),
            np.array([right,scene_center[1] - half_length, scene_center[2] + half_height]),
            np.array([right,scene_center[1] + half_length, scene_center[2] - half_height]),
            np.array([right,scene_center[1] - half_length, scene_center[2] - half_height])
        ]
    ...
    def right_angle_level_camera_generation1(self, w, l, h, scene_center, scan_pc):

        # Calculate fractions of dimensions
        half_width, half_length, half_height = w / 2, l / 2, h / 2

        # Define top and bottom z-coordinates
        bottom_0 = scene_center[0] + half_width
        bottom_1 = scene_center[1] - half_length
        bottom_2 = scene_center[2] - half_height
        ...
        # target_positions = [
        #     np.array([bottom_0, scene_center[1] + half_length, scene_center[2] + half_height]),
        #     np.array([bottom_0, scene_center[1] - half_length, scene_center[2] + half_height]),
        #     np.array([bottom_0, scene_center[1] + half_length, scene_center[2] - half_height]),
        #     np.array([bottom_0, scene_center[1] - half_length, scene_center[2] - half_height])
        # ]
        ...
        target_positions = [
            np.array([scene_center[0] + half_width, bottom_1, scene_center[2] + half_height] ),
            np.array([scene_center[0] + half_width, bottom_1, scene_center[2] - half_height]),
            np.array([scene_center[0] - half_width, bottom_1, scene_center[2] + half_height]),
            np.array([scene_center[0] - half_width, bottom_1, scene_center[2] - half_height])
        ]
...
def main():
    ...
    mask_path = "/output/replica/replica_ground_truth_masks/office0.pt"
